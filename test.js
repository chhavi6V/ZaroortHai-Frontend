const products = [
  ...
  products.map((product) => (console.log(product.images[0])))
  
  function getUniqueAvailabilityStatus(products) {
    const availabilitySet = new Set();
  
    products.forEach(item => {
      availabilitySet.add(item.warrantyInformation);
    });
  
    return Array.from(availabilitySet);
  }
  
  const uniqueAvailabilityStatus = getUniqueAvailabilityStatus(products);
  
  const abc = uniqueAvailabilityStatus.map((c) => ({
    value: c,
    label: c.split("-").join(" "),
    checked: false,
  }));
